...
values1 = df.values[15:19,6:8]
values2 = df.values[22:26,6:8]
...
rects21 = ax.bar(x - 0.5 * width, values1[:,1], width*width_ctr,hatch='///', label='Weight-reloading-16bit',color='#64666a', zorder = 2, bottom = values1[:,0])
rects22 = ax.bar(x + 0.5 * width, values2[:,1], width*width_ctr,hatch='///', label='Weight-reloading-32bit',color='#d4d4cb', zorder = 2, bottom = values2[:,0])
...
legend = plt.legend(bbox_to_anchor=(-0.32, 0.9, 1.40, 1), loc=3, shadow=False,mode='expand',ncol=1,fontsize='xx-large',frameon=False)
