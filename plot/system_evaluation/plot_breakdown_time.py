...
values1 = df.values[1:5,6:8]
values2 = df.values[8:12,6:8]
...
rects11 = ax.bar(x - 0.5 * width, values1[:,0], width*width_ctr, label='In-memory-inference-16bit',color='#935859', zorder = 2)
rects12 = ax.bar(x + 0.5 * width, values2[:,0], width*width_ctr, label='In-memory-inference-32bit',color='#9b9ca0', zorder = 2)
...
legend = plt.legend(bbox_to_anchor=(-0.288, 0.9, 1.0, 1), loc=3, shadow=False,mode='expand',ncol=1,fontsize='xx-large',frameon=False)
