...
# def createRoute999(cityList):
#     return random.sample(len(cityList))
...
def breedPopulation999(matingpool, eliteSize):
    children = []
    length = len(matingpool) - eliteSize
    pool = random.sample(matingpool, len(matingpool))
...
geneticAlgorithm999(popSize=100, eliteSize=20, mutationRate=0.01, generations=500, switchMat=switchMat)
